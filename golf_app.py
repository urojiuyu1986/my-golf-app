    ...
    with st.expander("新しい対戦結果を入力する", expanded=False):
        ...
        if in_opps:
            for opp in in_opps:
                ...
                opp_s = c1.number_input(f"{opp}のスコア (不明は0)", 0, 150, 0, key=f"s_{opp}")
                ...
                net_user_score = in_my_score - opp_hc if use_hc else in_my_score # 自分から引く
                
                auto_res_idx = 0
                if opp_s > 0: # 相手スコアがある時だけ判定
                    if net_user_score < opp_s: auto_res_idx = 0 
                    elif net_user_score > opp_s: auto_res_idx = 1
                    else: auto_res_idx = 2
                
                # 相手スコアが0/未入力ならHC適用中でも選択可能にする
                disable_select = True if (use_hc and opp_s > 0) else False
                res = c3.selectbox("結果", ["勝ち", "負け", "引き分け"], index=auto_res_idx, key=f"r_{opp}", disabled=disable_select)
...
if not h_df.empty:
    ...
    with st.expander("💾 履歴を直接編集・修正する"):
        edited = st.data_editor(v_df.drop(columns=['日付DT'], errors='ignore'), use_container_width=True, num_rows="dynamic", key="h_edit")
        if st.button("修正を反映"):
            if safe_save(edited, "history"): st.success("更新完了！"); st.rerun()
...
with st.sidebar:
    ...
    with st.expander("👤 友達を新規追加"):
        nf = st.text_input("名前")
        nh = st.number_input("初期HC", value=0.0)
        if st.button("友達保存"):
    ...
    with st.expander("📸 写真をアップロード"):
        if friend_names:
            tf = st.selectbox("対象", options=friend_names, key="side_p")
